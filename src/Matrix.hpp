 ...
 * */
    ...
    auto operator+(const Matrix<T1> &mat1, const T2 &t2) {
        vector<vector<Add_Result_t < T1, T2>> > temp(mat1.rows(), vector<Add_Result_t < T1, T2>>
        (mat1.cols()));
        ...
        return Matrix<Add_Result_t < T1, T2>>
        (std::move(temp));
    }
    ...
    template<typename T1, typename T2>
    auto operator-(const Matrix<T1> &mat1, const T2 &t2) {
        vector<vector<Minus_Result_t < T1, T2>> > temp(mat1.rows(), vector<Minus_Result_t < T1, T2>>
        (mat1.cols()));
        for (uint32_t i = 0; i < temp.size(); ++i) {
            for (uint32_t j = 0; j < temp[i].size(); ++j) {
                temp[i][j] = mat1.get_inside(i, j) - t2;
        ...
        return Matrix<Minus_Result_t < T1, T2>>
        (std::move(temp));
    }
 ...
 * */
    ...
    Matrix<T> Matrix<T>::inverse() const {
        if (this->is_square() && this->determinant() != 0) {//可逆条件
            ...
            for (int32_t i = 0; i < this->rows(); i++) {
                for (int32_t j = 0; j < this->cols(); j++) {
                    ...
                    for (int32_t k = 0; k < i - 1; k++) {//左上
                        for (int32_t t = 0; t < j - 1; t++) {
                            submatrix[k][t] = vec[k][t];
                        }
                    }
                    for (int32_t k = 0; k < i - 1; k++) {//右上
                        for (int32_t t = j; t < this->cols() - 1; t++) {
                            submatrix[k][t] = vec[k][t + 1];
                        }
                    }
                    for (int32_t k = i; k < this->rows() - 1; k++) {//左下
                        for (int32_t t = 0; t < j; t++) {
                            submatrix[k][t] = vec[k + 1][t];
                        }
                    }
                    for (int32_t k = i; k < this->rows() - 1; k++) {//右下
                        for (int32_t t = j; t < this->cols() - 1; t++) {
                            submatrix[k][t] = vec[k + 1][t + 1];
                        }
                    }
                    res[i][j] = (-1) ^ (i + j) * determinant_in(submatrix);//子矩阵展开得到伴随矩阵
                }
            }
